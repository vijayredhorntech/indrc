...
@section('content')
    ...
    <form class=" {{$formData['type']==='show'?'d-none':''}}" action="{{$formData['url']}}" method="{{$formData['method']}}"  enctype="multipart/form-data" id="addNewStat">
        @csrf
                ...
                <div class="col-12 col-xl-4">
                    <div class="card mb-3">
                        <div class="card-body">
                            ...
                            <input class="form-control mb-5" type="file" name="image[]" multiple accept="image/*">
                            @error('image.*')
                            <span class="text-danger">{{ $message }}</span>
                            @enderror
                            <div class="w-full d-flex justify-content-end">
                                <button class="btn btn-primary mb-2 mb-sm-0" type="submit">
                                    {{$formData['type']==='show'?'Add Banner Image':'Update Banner Image'}}
                                </button>
                            </div>
                        </div>
                    </div>
                ...
                <div class="table-responsive scrollbar mx-n1 px-1">
                    <table class="table table-sm fs-9 mb-0">
                        ...
                        <tr>
                            ...
                            <th class="sort align-middle ps-8" scope="col" data-sort="customer" style="width:30%;">IMAGE</th>
                        ...
                        @forelse($bannerStats as $key => $stat)
                            <tr class="hover-actions-trigger btn-reveal-trigger position-static">
                                ...
                                <td class="customer align-middle white-space-nowrap ps-8">
                                    @if($stat->image)
                                        <img style="height: 80px;" src="{{ asset( $stat->image) }}" alt="Banner Image">
                                    @endif
                                </td>
